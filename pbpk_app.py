...
with tab_sim:
    ...
    with right:
        ...
        st.metric("Cmax", f"{float(np.max(c)):.4g}")
        st.metric("Tmax (h)", f"{float(t[np.argmax(c)]):.4g}")
        st.metric("AUC0–t", f"{auc_trapz(t, c):.4g}")

with tab_pk:
    st.subheader("PK indicators + Full NCA")
    ...
    # Infer tau from the most recent positive interval
    tau = None
    ...
    if len(sub) < 2:
        st.warning("Window has insufficient points for NCA.")
    else:
        # time shifted to window start for NCA reporting
        t_win = (sub.time_h.to_numpy() - float(t0))
        c_win = sub[series].to_numpy()

        # dose at window start (for CL/F and Vz/F; best when using "Last tau")
        dose_at_t0 = None
        ...
        # Terminal phase
        st.markdown("**Terminal phase (λz / t½)**")
        n_term = st.slider("Max terminal points (choose from last 3..N)", min_value=3, max_value=12, value=6, step=1)
        ...
        if lam_info is not None:
            ...
            nca["Terminal n"] = lam_info["n_points"]
            ...
            if np.isfinite(clast) and clast >= 0:
                ...
                # Apparent CL/F and Vz/F if dose known
                if dose_at_t0 is not None and nca["AUC0-inf"] > 0:
                    # If units are ng/mL or nM, we treat AUC in "conc·h" and CL/F is "dose / AUC"
                    # For ng/mL: convert AUC to mg·h/L: (ng/mL)*h * (1 mg / 1e6 ng) * (1000 mL / 1 L) = /1000
                    if series.endswith("(ng/mL)") or series.endswith("(ng/mL eq)"):
                        auc_mg_h_L = float(nca["AUC0-inf"]) / 1000.0
                        nca["CL/F (L/h)"] = float(dose_at_t0 / auc_mg_h_L) if auc_mg_h_L > 0 else float("nan")
                        nca["Vz/F (L)"] = float(nca["CL/F (L/h)"] / lam) if np.isfinite(lam) else float("nan")
                    else:
                        # For nM, CL/F in "dose / (nM*h)" is not directly physical without unit conversion to mass.
                        nca["CL/F (note)"] = "Not computed in nM (needs MW-based mass conversion)."

        # Display NCA table + key metrics
        st.divider()
        st.subheader("Full NCA table")

        order = [
            "Dose (mg)",
            "Cmax","Tmax","Cmin","Tmin","Clast","Tlast",
            "AUC0-last","AUC_extra","AUC0-inf","%AUC_extrap",
            "Cavg","Swing%","Peak/Trough",
            "lambda_z","t_half","R2 (terminal fit)","Terminal n",
            "AUMC0-last","AUMC_extra","AUMC0-inf","MRT0-inf",
            "CL/F (L/h)","Vz/F (L)","CL/F (note)"
        ]
        rows = []
        for k in order:
            if k in nca:
        ...
        if lam_info is not None:
            with st.expander("Show terminal phase fit (log-linear)", expanded=False):
                ...
                t_seg, c_seg = t_pos[-int(lam_info["n_points"]):], c_pos[-int(lam_info["n_points"]):]
                ln_c = np.log(c_seg)
...
- NCA is computed on the selected **PK window**.
