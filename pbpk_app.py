...
with st.sidebar:
    ...
    if schedule_type == "Every X hours (qXh)":
        ...
        # Maintenance starts automatically after the last loading dose
    maint_start = float(max(1, int(n_ld)) * float(ld_int))
* 24.0,
...
if 'units' in locals() and units == "nM":
    mw = float(mw_g_mol)
...
with tab_sim:
    ...
    with right:
        ...
        plasma = "Plasma (nM)" if ('units' in locals() and units=="nM") else "Plasma (ng/mL)"
...
with tab_pk:
    ...
    series = st.selectbox("Series", options=[c for c in df.columns if c.endswith("ng/mL)") or c.endswith("ng/mL eq)") or c.endswith("(nM)") or c.endswith("(nM eq)")], index=0)
    ...
    st.divider()
    st.subheader("Full NCA (noncompartmental analysis)")

    st.caption("NCA is computed on the **selected window** above for the chosen series. "
               "Terminal phase parameters use a log-linear fit on the last N points with positive concentrations.")

    # Build the selected window arrays (shift time to start at 0 for reporting)
    sub = df[(df.time_h >= t0) & (df.time_h <= t1)].copy()
    t_win = (sub.time_h.to_numpy() - float(t0))
    c_win = sub[series].to_numpy()

    # Identify the dose associated with the window start (if applicable)
    # For 'Last dosing interval (tau)' this is usually the last dose at t0.
    dose_at_t0 = None
    for tt, dd in dose_events:
        if abs(float(tt) - float(t0)) < 1e-6:
            dose_at_t0 = float(dd)
            break

    n_term = st.slider("Terminal points (N)", min_value=3, max_value=10, value=6, step=1)
    auto_term = st.checkbox("Auto-select terminal points (best R²)", value=True)
    lam_info = terminal_slope_auto(t_win, c_win, n_min=3, n_max=int(n_term)) if auto_term else terminal_slope(t_win, c_win, n_points=int(n_term))

    nca = nca_full(t_win, c_win, dose_mg=dose_at_t0)

    # Add terminal-derived metrics if possible
    if lam_info is not None:
        lam = lam_info["lambda_z"]
        nca["lambda_z"] = lam
        nca["R2 (terminal fit)"] = lam_info["r2"]
        nca["t_half"] = float(np.log(2.0) / lam)

        # Extrapolation
        clast = nca.get("Clast", float("nan"))
        if np.isfinite(clast) and clast >= 0:
            nca["AUC_extra"] = float(clast / lam)
            nca["AUC0-inf"] = float(nca["AUC0-last"] + nca["AUC_extra"])
            nca["%AUC_extrap"] = float(100.0 * nca["AUC_extra"] / nca["AUC0-inf"]) if nca["AUC0-inf"] > 0 else float("nan")

            # AUMC extrap + MRT
            tlast = nca.get("Tlast", float("nan"))
            nca["AUMC_extra"] = float((clast * tlast) / lam + clast / (lam ** 2))
            nca["AUMC0-inf"] = float(nca["AUMC0-last"] + nca["AUMC_extra"])
            nca["MRT0-inf"] = float(nca["AUMC0-inf"] / nca["AUC0-inf"]) if nca["AUC0-inf"] > 0 else float("nan")

            # Apparent clearance and volume if dose known (oral: CL/F, Vz/F)
            if dose_at_t0 is not None and nca["AUC0-inf"] > 0:
                nca["Dose (mg)"] = float(dose_at_t0)
                nca["CL/F (L/h)"] = float(dose_at_t0 / (nca["AUC0-inf"] / 1000.0))  # AUC in ng·h/mL -> mg·h/L via /1000
                nca["Vz/F (L)"] = float(nca["CL/F (L/h)"] / lam)
    ...
    order = [
        ...
        "Cmax", "Tmax",
        "Clast", "Tlast",
        "AUC0-last", "AUC0-inf", "AUC_extra", "%AUC_extrap",
        "Cavg",
        "Swing%", "Peak/Trough",
        "lambda_z", "t_half", "R2 (terminal fit)",
        "AUMC0-last", "AUMC0-inf", "MRT0-inf",
        "CL/F (L/h)", "Vz/F (L)",
    ]
    rows = []
    for k in order:
        if k in nca:
            rows.append({"Metric": k, "Value": nca[k]})
    # add any extras not in order
    for k, v in nca.items():
        if k not in set(order):
            rows.append({"Metric": k, "Value": v})

    nca_df = pd.DataFrame(rows)
    st.dataframe(nca_df, hide_index=True, use_container_width=True)

    # Optional: show terminal fit plot
    if lam_info is not None:
        with st.expander("Show terminal phase fit (log-linear)", expanded=False):
            # Use the last N points used by the fit
            mask = np.isfinite(t_win) & np.isfinite(c_win) & (c_win > 0)
            t_pos, c_pos = t_win[mask], c_win[mask]
            t_seg, c_seg = t_pos[-int(n_term):], c_pos[-int(n_term):]
            ln_c = np.log(c_seg)
            b, a = np.polyfit(t_seg, ln_c, 1)
            ln_pred = a + b * t_seg

            fig2 = go.Figure()
            fig2.add_trace(go.Scatter(x=t_win, y=c_win, mode="lines", name="Conc"))
            fig2.add_trace(go.Scatter(x=t_seg, y=c_seg, mode="markers", name="Terminal pts"))
            # Plot fitted line in concentration space
            c_fit = np.exp(ln_pred)
            fig2.add_trace(go.Scatter(x=t_seg, y=c_fit, mode="lines", name="Fit (terminal)"))
            fig2.update_layout(height=360, margin=dict(l=10,r=10,t=10,b=10),
                               xaxis_title="Time in window (h)", yaxis_title="Concentration",
                               legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0))
            fig2.update_yaxes(type="log")
            st.plotly_chart(fig2, use_container_width=True)
            st.caption(f"lambda_z={lam_info['lambda_z']:.4g} 1/h, half-life={float(np.log(2)/lam_info['lambda_z']):.3g} h, R²={lam_info['r2']:.3g}")
...
def terminal_slope(t: np.ndarray, c: np.ndarray, n_points: int = 3) -> Optional[Dict[str, float]]:
    """Estimate terminal slope (lambda_z) via log-linear regression on last n positive points."""
    t = np.asarray(t, dtype=float)
    c = np.asarray(c, dtype=float)
    mask = np.isfinite(t) & np.isfinite(c) & (c > 0)
    t, c = t[mask], c[mask]
    if len(t) < max(3, n_points):
        return None
    # take last n_points
    t_seg = t[-n_points:]
    c_seg = c[-n_points:]
    ln_c = np.log(c_seg)
    # Linear regression ln(c) = a + b*t
    b, a = np.polyfit(t_seg, ln_c, 1)
    # b should be negative; lambda_z = -b
    lam = float(-b)
    ...
    return {"lambda_z": lam, "r2": float(r2), "n_points": int(n_points),
...
def terminal_slope_auto(t: np.ndarray, c: np.ndarray, n_min: int = 3, n_max: int = 10) -> Optional[Dict[str, float]]:
    """Pick the best terminal slope among last n points (n_min..n_max) by highest R², preferring more points on ties."""
    best = None
    for n in range(int(n_min), int(n_max) + 1):
        ...
        if best is None:
            ...
            if (info.get("r2", -np.inf) > best.get("r2", -np.inf) + 1e-9) or (
                abs(info.get("r2", -np.inf) - best.get("r2", -np.inf)) <= 1e-9 and info.get("n_points", 0) > best.get("n_points", 0)
            ):
                best = info
    return best
...
def nca_full(t: np.ndarray, c: np.ndarray, dose_mg: Optional[float] = None) -> Dict[str, float]:
    ...
    # Linear trapezoid AUC and AUMC
    out["AUC0-last"] = auc_trapz(t, c)
    out["AUMC0-last"] = auc_trapz(t, t * c)
    out["Cavg"] = float(out["AUC0-last"] / (t[-1] - t[0])) if (t[-1] - t[0]) > 0 else float("nan")

    # Terminal phase
    # We'll fill lambda_z later from UI choice; placeholders here
    return out
