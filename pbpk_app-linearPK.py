...
def build_excel_export(df: pd.DataFrame, dose_events: List[Tuple[float, float]], p: Params, pk_summary: Dict[str, float], series: str) -> bytes:
...
with tab_pk:
    ...
    xlsx_bytes = build_excel_export(df, dose_events, p, pk, series)
...
- Downloaded CSV/XLSX files now include the updated model outputs plus dose events and a PK summary sheet.
