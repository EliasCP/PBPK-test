...
def build_simulation_csv(df: pd.DataFrame, dose_events: List[Tuple[float, float]], p: Params, series: str) -> str:
    """Build a CSV string with metadata rows followed by the simulation timecourse."""
    ...
    meta_rows = [
        ...
        ["series", series],
        ["dose_regimen", df["Dose regimen"].iloc[0] if "Dose regimen" in df.columns and len(df) else ""],
        ["dose_events_n", len(dose_events)],
        ["loading_dose_mg", df["Loading dose (mg)"].iloc[0] if "Loading dose (mg)" in df.columns and len(df) else ""],
        ["maintenance_dose_mg", df["Maintenance dose (mg)"].iloc[0] if "Maintenance dose (mg)" in df.columns and len(df) else ""],
        ["loading_doses", df["Loading doses"].iloc[0] if "Loading doses" in df.columns and len(df) else ""],
        ["loading_interval_h", df["Loading interval (h)"].iloc[0] if "Loading interval (h)" in df.columns and len(df) else ""],
        ["simulation_horizon_days", df["Simulation horizon (days)"].iloc[0] if "Simulation horizon (days)" in df.columns and len(df) else ""],
        ["model_params", ""],
    ]

    param_rows = [[f"param:{k}", getattr(p, k)] for k in sorted(p.__dict__.keys())]
    buf = StringIO()
    pd.DataFrame(meta_rows + param_rows).to_csv(buf, index=False, header=False)
    buf.write("\n")
    df.to_csv(buf, index=False)
    return buf.getvalue()


def build_nca_csv(nca_tissue_df: pd.DataFrame, dose_events: List[Tuple[float, float]], p: Params) -> str:
    """Build a CSV string for tissue NCA results with metadata rows on top."""
    from io import StringIO
    ...
    meta_rows = [
        ...
        ["model_label", MODEL_LABEL],
        ["dose_events_n", len(dose_events)],
        ["tissue_series_n", len(nca_tissue_df)],
        ["model_cl_livC", getattr(p, 'CL_livC')],
        ["fixed_mw_g_mol", 474.5961],
        ["nca_section", "full_simulation"],
    ]

    buf = StringIO()
    pd.DataFrame(meta_rows).to_csv(buf, index=False, header=False)
    buf.write("\n")
    nca_tissue_df.to_csv(buf, index=False)
    return buf.getvalue()
...
with tab_pk:
    ...
    nca_csv = build_nca_csv(nca_tissue_df, dose_events, p)
...
- Downloaded CSV files now include the updated model outputs plus dose events, model metadata, and a tissue-level NCA section.
