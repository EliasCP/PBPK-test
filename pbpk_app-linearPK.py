...
def build_excel_export(df: pd.DataFrame, dose_events: List[Tuple[float, float]], p: Params, pk_summary: Dict[str, float], series: str, nca_tissue_df: Optional[pd.DataFrame] = None) -> bytes:
    """Create a multi-sheet Excel export with model, regimen, and summary details."""
    from io import BytesIO

    buf = BytesIO()
    params_df = pd.DataFrame([
        {"parameter": k, "value": getattr(p, k)}
        for k in sorted(p.__dict__.keys())
    ])
    regimen_df = pd.DataFrame(dose_events, columns=["time_h", "dose_mg"])
    summary_df = pd.DataFrame([{"metric": k, "value": v} for k, v in pk_summary.items()])
    meta_df = pd.DataFrame([
        {"field": "model_label", "value": MODEL_LABEL},
        {"field": "series", "value": series},
        {"field": "rows", "value": len(df)},
        {"field": "columns", "value": len(df.columns)},
    ])
    if nca_tissue_df is None:
        tissue_cols = [c for c in df.columns if c.endswith("(ng/mL)") or c.endswith("(ng/mL eq)") or c.endswith("(nM)") or c.endswith("(nM eq)")]
        nca_tissue_df = tissue_nca_table(df, tissue_cols)
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        ...
        regimen_df.to_excel(writer, index=False, sheet_name="dose_events")
        summary_df.to_excel(writer, index=False, sheet_name="pk_summary")
        nca_tissue_df.to_excel(writer, index=False, sheet_name="tissue_nca")
...
with tab_pk:
    ...
    xlsx_bytes = build_excel_export(df, dose_events, p, pk, series, nca_tissue_df=nca_tissue_df)
    ...
    with c2:
        st.download_button(
            ...
            data=xlsx_bytes,
            file_name="pbpk_simulation_updated.xlsx",
...
- Downloaded CSV/XLSX files now include the updated model outputs plus dose events, model metadata, and a PK summary sheet.
- The Excel export also includes a tissue-level NCA sheet.
